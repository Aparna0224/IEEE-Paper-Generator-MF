...
class GenerationRequest(BaseModel):
    ...
    model_name: str = Field(default="llama-3.1-8b-instant", description="Groq model to use")
...
async def generate_paper_task(task_id: str, topic: str, max_results: int, model_name: str, authors: list[dict]):
    ...
    try:
        ...
        generation_status[task_id].update({
            "status": "failed",
            "progress": 0,
            "message": "All Groq LLM models failed. Check GROQ_API_KEY and model availability.",
            "error": str(e),
            "updated_at": datetime.now().isoformat()
        })

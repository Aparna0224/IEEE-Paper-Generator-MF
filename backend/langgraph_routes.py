...
default_model = os.getenv("GROQ_MODEL", os.getenv("DEFAULT_MODEL", "llama-3.1-8b-instant"))
groq_configured = bool(groq_api_key)

if groq_configured:
    print(f"[MODEL] Using Groq model: {default_model}")
else:
...
async def generate_paper(request: PaperGenerationRequest) -> PaperGenerationResponse:
    ...
    try:
        if graph_service is None:
            raise HTTPException(
                status_code=503,
                detail="API service not initialized. Groq backend is unavailable.",
            )
